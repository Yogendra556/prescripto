...
const MyProfile = () => {
  const [userData, setUserData] = useState({
    ...
    email: "Hlljs@email.com",
    phone: "+91 99278 97898",
    address: {
      line1: "Civil road",
      line2: "Kanpur"
    },
  ...
  return (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      ...
      <div className='mt-10'>
      {
        isEdit ? <button className='border border-[#5f6fff] px-8 py-2 rounded-full hover:bg-[#5f6fff] hover:text-white transition-all duration-200' onClick={()=>{setIsEdit(false)}}>Save Changes</button>:
        <button className='border border-[#5f6fff] px-8 py-2 rounded-full hover:bg-[#5f6fff] hover:text-white transition-all duration-200'  onClick={()=>{setIsEdit(true)}}>Edit</button>
      }
      </div>
    </div>
  )
